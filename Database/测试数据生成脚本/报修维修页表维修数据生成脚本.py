...
for i in range(100):
        ...
        FullName = XM()
        LastName = ''.join(pypinyin.pinyin(FullName, style=pypinyin.NORMAL)[0])
        FirstName = ''.join(pypinyin.pinyin(FullName, style=pypinyin.NORMAL)[1] +
                                    pypinyin.pinyin(FullName, style=pypinyin.NORMAL)[2])

        mysheet[chr(col) + str(row)] = str(i)
        col += 1
        mysheet[chr(col) + str(row)] = FullName
        col += 1
        mysheet[chr(col) + str(row)] = ''.join(random.sample('112233445566778899', 11))
        col += 1
        mysheet[chr(col) + str(row)] = '2022-02-28 13:41:41'
        col += 1
        values = ['电', '水', '能源', '其他']
        mysheet[chr(col) + str(row)] = random.choice(values)
        col += 1
        values2 = ['13号楼1楼2单元走廊灯', '中心广场喷泉', '门卫收发室', '5号楼五楼3单元1号电梯', '10号楼711室', '小区东门', '服务中心菜鸟驿站']
        mysheet[chr(col) + str(row)] = random.choice(values2)
        col += 1
        values3 = ['灯光很暗，即将结束使用寿命', '西南侧管道漏水', '暖气片漏水', '电梯开不了门', '水管破裂', '盆栽被偷', '火灾', '洪水', '火山爆发', '泥石流']
        mysheet[chr(col) + str(row)] = random.choice(values3)
        col += 1
        mysheet[chr(col) + str(row)] = XM()
        col += 1
        values4 = ['正在维修', '正在前往', '已维修', '待派发']
        mysheet[chr(col) + str(row)] = random.choice(values4)
        col += 1
        mysheet[chr(col) + str(row)] = '2022-02-28 15:41:41'
        row += 1
